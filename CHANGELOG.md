...
- **Global Tool Distribution:** Support for zero-clone single-command installation via `uv tool install git+...` and `pipx install git+...`.
...
### Changed
- **Documentation & Setup UX:** Enhanced user guide with explicit directory navigation, `uv sync` dependency preparation, and simplified global CLI commands (`pocketrelay init`, `pocketrelay run`).
